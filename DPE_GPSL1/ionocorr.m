function ionodel = ionocorr(systime,svxyz,usrxyz,settings)
%IONOCORRCompute ionospheric correction from the
...
%	parameters (settings.ALPHA and settings.BETA vectors) have been loaded
%	into global memory
%
%	OUTPUTS
...
%
% global settings.ALPHA settings.BETA
...
PER = settings.BETA(1) + settings.BETA(2)*phim + settings.BETA(3)*phim^2 + settings.BETA(4)*phim^3;
if PER < 72000, PER = 72000; end
x = 2*pi*(t - 50400)/PER;
	
AMP = settings.ALPHA(1) + settings.ALPHA(2)*phim + settings.ALPHA(3)*phim^2 + settings.ALPHA(4)*phim^3;
if AMP < 0, AMP = 0; end

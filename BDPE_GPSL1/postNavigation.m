...
%% Find satellites positions, velocity, and clocks corrections =======================
    % Outputs are all colume vectors corresponding to activeChnList
    % Code changed to also compute satellite velocity
    % Basically uncommented the satvel and clkdrift parts of satpos.m
    % Added by Sergio Vicenzo - 10 June 2025
    [satPositions, satClkCorr, satVelocity, satClkCorrRat] = ...
                                satpos(transmitTime(activeChnList), ...
                                 [trackResults(activeChnList).PRN], eph); 

    % [satPositions2,  satVelocity2,satClkCorr2] = satPosVel(transmitTime, ...
    %     [trackResults(1:length(transmitTime)).PRN],eph,length(activeChnList),activeChnList); 
  

    % Record the satellites position velocity and clocks corrections
    navSolutions.satPositions = satPositions;
    navSolutions.satVelocity  = satVelocity;
...
%% Coordinate conversion ==================================================
    ...
    tic
            localTime_iono = localTime - xyzdt(4)/settings.c;  
    
    [navSolutions] = DPE_module_PF7...
        (currMeasNr,navSolutions,activeChnList,...
        trackResults,currMeasSample,satPositions,...
        transmitTime(activeChnList),localTime_iono,...
        settings,satElev,fid,xyzdt(4),satClkCorr,satVelocity,codePhase,...
        satClkCorrRat,eph);
...
%     Added by Sergio Vicenzo - 14 Feb 2024
    ...
    fprintf('\nCurrent 3D Error of BDPE     : %1f\n',...
        (navSolutions.LLH_error(currMeasNr,4)));
    fprintf('Current 3D Error of LS      : %1f\n',...
        (navSolutions.LLH_error(currMeasNr,3)));

     single_diff_pseudorange = ...
        sqrt(sum((satPositions'-repmat(GT_ECEF,length(satPositions(1,:)),1)).^2,2));

    navSolutions.single_diff_error(:,currMeasNr)=...
        navSolutions.correctedP(activeChnList, currMeasNr) - single_diff_pseudorange;

...
%% Find receiver position =================================================
    ...
    if size(activeChnList, 2) > 3
        ...
        navSolutions.LS_processingtime(currMeasNr) = toc;
...
%% Coordinate conversion ==================================================
    ...
    [navSolutions] = CorrDPE_ARS...
        ...
        settings,satElev,xyzdt(4),satClkCorr);
...
%     Added by Sergio Vicenzo - 14 Feb 2024
    fprintf('\nCurrent 2D Error of Corr-DPE : %1f\n',...
        (navSolutions.LLH_error(currMeasNr,2)));
    ...
    fprintf('\nCurrent 3D Error of Corr-DPE : %1f\n',...
        (navSolutions.LLH_error(currMeasNr,4)));
...
save([settings.outfile_root, '\navSol_CorrDPE_',settings.outfile_root,...
    '_DLLspacing_',...
    num2str(settings.dllCorrelatorSpacing),'chips_cohInt',...
    num2str(settings.DPE_cohInt),'ms_multicorrSpacing_',...
    num2str(settings.chipspacing_dpe_precalc)...
    ,'chips_DATE_',today_date,'.mat'],...
...
legend('Least Squares (Scalar Tracking)','Corr-DPE','GT');

saveas(gcf, [pwd ,'\',settings.outfile_root,'\',...
    settings.outfile_root,'_CorrDPE_geoplot.png']);
saveas(gcf, [pwd ,'\',settings.outfile_root,'\',...
    settings.outfile_root,'_CorrDPE_geoplot']);
...
bc.Color = '#FF9200';
bc.LineWidth = 3;
legend('Least Squares (Scalar Tracking)','Corr-DPE','FontSize',12);
...
saveas(gcf, ...
    ...
    settings.outfile_root,'_CorrDPE_2DPositioning Error']);
saveas(gcf, ...
    [pwd ,'\',settings.outfile_root,'\',...
    settings.outfile_root,'_CorrDPE_2DPositioning Error.png']);
...
saveas(gcf, ...
    [pwd ,'\',settings.outfile_root,'\',...
    settings.outfile_root,'_CorrDPE_3DPositioning Error']);
saveas(gcf, ...
    [pwd ,'\',settings.outfile_root,'\',...
    settings.outfile_root,'_CorrDPE_3DPositioning Error.png']);

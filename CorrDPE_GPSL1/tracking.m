...
% Multicorrelator outputs
% Added by Sergio Vicenzo - 17 Feb 2025
trackResults.MultiCorr_I = zeros(1, codePeriods);
trackResults.MultiCorr_Q = zeros(1, codePeriods);
trackResults.MultiCorr_IQ = zeros(1, codePeriods);
...
% Added by Sergio Vicenzo - 17 Feb 2025
trackResults.tracking_time      = nan(1, codePeriods);
...
%             end
            ...
            tcode       = (remCodePhase-0.5) : ...
                codePhaseStep : ...
                ((blksize-1)*codePhaseStep+remCodePhase-0.5);
            tcode2      = ceil(tcode) + 1;
...
%             end
        ...
        for loopCnt =  1:codePeriods
            ...
            % Moved here by Sergio Vicenzo to compute multicorrelators
            % 17 Feb 2025
            %% Generate the carrier frequency to mix the signal to baseband -----------
            % Save remCarrPhase for current correlation
            trackResults(channelNr).remCarrPhase(loopCnt) = remCarrPhase;
            ...
            tcode_o       = (remCodePhase) : ...
                codePhaseStep : ...
                ((blksize-1)*codePhaseStep+remCodePhase);

            remCodePhase_new = ...
                (tcode_o(blksize) + codePhaseStep) - ...
                settings.codeLength* (settings.DPE_cohInt);

            count=1;

            %% Generate multicorrelators ----------------------------------------------
            % Added by Sergio Vicenzo - 17 Feb 2025
            for multicorr_spacing=...
                    [-(flip(settings.chipspacing_dpe_precalc:...
                ...
                tcode = tcode_o - multicorr_spacing;
                ...
                trackResults(channelNr).MultiCorr_I(count,loopCnt) = ...
                      I;
                trackResults(channelNr).MultiCorr_Q(count,loopCnt) = ...
                      Q;
                trackResults(channelNr).MultiCorr_IQ(count,loopCnt) = ...
                      sqrt(I.^2 + Q.^2);

                if multicorr_spacing == earlyLateSpc
%                     earlyCode   = s;
                    I_E = I;
                    Q_E = Q;
                elseif multicorr_spacing == -earlyLateSpc
%                     lateCode   = s;
                    I_L = I;
                    Q_L = Q;
                elseif multicorr_spacing == 0
%                     promptCode   = s;
                    I_P = I;
                    Q_P = Q;
                end
...
%             Q_L = sum(lateCode   .* qBasebandSignal);
            ...
            % Added by Sergio Vicenzo to evaluate tracking time with
            % multicorrelators
            % 17 Feb 2025
            trackResults(channelNr).tracking_time(loopCnt) = toc;
        end % for loopCnt

...
%% Acquisition ============================================================
    ...
    if ~exist([settings.outfile_root, '\acqResults_gps_', settings.outfile_root...
                    ,'.mat'])
        ...
        data  = fread(fid, dataAdaptCoeff*400*samplesPerCode, settings.dataType)';
...
%         acqResults = acquisition(data, settings); % Replaced with a
%         higher sensitvity code - 8 Dec 2025
        acqResults = acquisition_DENNISAKOS(data, settings);

        plotAcquisition(acqResults);

        save([settings.outfile_root, '\acqResults_gps_', ...
           ...
           load([settings.outfile_root, '\acqResults_gps_',...
                settings.outfile_root,'.mat']);
...
if ~exist([settings.outfile_root, '\trackResults_gps_', settings.outfile_root...
                ...
                num2str(settings.DPE_cohInt),'_',...
                num2str(settings.chipspacing_dpe_precalc*100),...
                '.mat'])
    startTime = now;
    ...
    % Added by Sergio Vicenzo - 06 Jan 2025
     save([settings.outfile_root, '\trackResults_gps_', ...
                ...
                num2str(settings.DPE_cohInt),'_',...
                num2str(settings.chipspacing_dpe_precalc*100),'.mat'],...

...
%
% DPE_module is a Direct Position Estimation (DPE)
% plug-in module that can be integrated into existing two-step positioning 
% MATLAB SDRs.
%
...
%
% The plug-in module uses 1 ms for both coherent and non-coherent
% integration
%
...
% === Repeat lat-long-clkbias search space over height search space =======
  for alt = alt_search
    ...
    for j=1:length(activeChnList)
        ...
        codePhase3 = (codePhase4)+codePhase(activeChnList(j));


        % === Ensures that the code phase of each candidate position ======
        % ===============is within 0 and chip length ======================
        codePhase3(codePhase3>settings.codeLength)=...
            abs(settings.codeLength-...
            codePhase3(codePhase3>settings.codeLength));

        codePhase3(codePhase3<0)=...
            (settings.codeLength+codePhase3(codePhase3<0));
    ...
    % === Obtain the lat-long-clock bias estimate for a specific height ===
       ...
       % In the case there is more than one global maxima over the
       % navigation domain, the mean of those coordinates with max
       % correlation value will be used
        if length(I) ~= 1 
            ...
            temprecord_DPE_values(barisan,7) = mean(I);
        else
